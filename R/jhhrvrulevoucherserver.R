...
viewrulevoucherserver <- function(input,output,session,dms_token) {
  ...
  shiny::observe({
    shiny::observeEvent(input$btn_hrv_ods_view_rulevoucher,
                        {
                            sql = 'select * from rds_hrv_ods_rule_voucher'
                            ...
                            #显示数据
                            tsui::run_dataTable2(id = 'hrv_ods_view_data_rulevoucher', data = data)
